...
parser.add_argument("model_name", help="timm_model_type", default="swin_s3_tiny")
parser.add_argument("inp_channels", help="use channels if RGB, set 3", default=3)
parser.add_argument("num_img_feature", help="timm model output feature num from back bone model", default=5)
parser.add_argument("pretrained", help="if use timm pretrained model, set True", default=True)

parser.add_argument("n_epoch", help="training epoch num", default=10)
parser.add_argument("n_folds", help="cross validation fold num", default=3)
parser.add_argument("train_folds", help="use folds list", default=[0, 1, 2])
...
parser.add_argument("batch_size", help="train/valid batchsize", default=128)
parser.add_argument("num_workers", default=2)
parser.add_argument("print_freq", help="training situation print freq", default=1000)
...
if not args.kaggle:
    ...
    MODEL_DIR = os.path.join(OUTPUT_DIR, args.EXP_NAME ,"model")
    CONTACT_IMG_DIR = os.path.join(INPUT_DIR, "contact_images")
...
class NFLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        contact_filename = os.path.join(args.CONTACT_IMG_DIR, contact_fileid)
...
class NFLNet(nn.Module):
    def __init__(
        ...
        out_features = args.num_img_feature,
...
def valid_fn(model, valid_loader, criterion):
    ...
    for batch_idx, (images, targets, is_End) in enumerate(valid_loader):
        ...
        if batch_idx % args.print_freq == 0 or batch_idx == (len(valid_loader)-1):
            print('\t EVAL: [{0}/{1}] '
                'Elapsed {remain:s} '
                'Loss: {loss.val:.4f}({loss.avg:.4f}) '
                .format(
...
def training_loop(target_df):
    ...
    for fold, (idx_train, idx_valid) in enumerate(kf.split(target_df, target_df["contact_id"], target_df["game_play"])):
        ...
        optimizer = AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay, amsgrad=False)
        scheduler = CosineAnnealingLR(optimizer, T_max=args.T_max, eta_min=args.min_lr, last_epoch=-1)

        if not fold in args.train_folds:
        ...
        for epoch in range(1, args.n_epoch + 1):
            ...
            if valid_score > best_score:
                ...
                img_emb_colname = [f"img_emb_{idx}" for idx in range(args.num_img_feature)]
