...
CFG = {
        ...
        "out_features" : 20,
        "use_frames" : [-6, 0, 6],
        "inp_channels": 3*2*3,# rgb*view*frames
        "pretrained" : True,
        ...
        "EXP_NAME" : "baseline026fmlp1d_views_frames",
}
...
class NFLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_img = None
        for load_relative_frame in CFG["use_frames"]:
            load_frame = frame + load_relative_frame
            if CFG["kaggle"]:
                file_id = f"{game_play}_Endzone_{load_frame:05}.png"
            else:
        ...
        for load_relative_frame in CFG["use_frames"]:
            load_frame = frame + load_relative_frame
            if CFG["kaggle"]:
                file_id = f"{game_play}_Sideline_{load_frame:05}.png"
            else:
                file_id = f"{game_play}_Sideline_{load_frame:04}.jpg"
            filename = os.path.join(CFG["TRAIN_E_IMG_DIR"], file_id)
            img = cv2.imread(filename)
            if img is None:
                img = np.zeros((224, 224, 3))
